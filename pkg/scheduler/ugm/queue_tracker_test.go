...
func TestQTSetMaxApps(t *testing.T) {
	...
	// root->parent->child12 (similar name like above leaf queue, but it is being treated differently as similar names are allowed)
	queueTracker := newQueueTracker("root")
	queueTracker.setMaxApplications(100, queuePath1)
	queueTracker.setMaxApplications(1000, queuePath2)

	usage := queueTracker.getResourceUsageDAOInfo("")
	resources := make(map[string]uint64)

	println(internalGetMaxRunningApplications(usage, resources)[queuePath1])
	println(internalGetMaxRunningApplications(usage, resources)[queuePath2])
}
...
func TestQTSetMaxResource(t *testing.T) {
	...
	queueTracker := newQueueTracker("root")
	queueTracker.setMaxResources(usage1, queuePath1)
	queueTracker.setMaxResources(usage1, queuePath2)

	resources := getQTMaxResource(queueTracker)

	println(resources[queuePath1].String())
	println(resources[queuePath2].String())
